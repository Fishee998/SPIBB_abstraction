...
def compute_mask_N_wedge(nb_states, nb_actions, N_wedge, batch, abstract_map=None):
    ...
    if abstract_map:
        count_state_action_mask = np.zeros((nb_states, nb_actions), dtype=bool)
        for action in range(nb_actions):
            for states in abstract_map:
                count = 0
                for state in states:
                    count += count_state_action[state, action]
                if count > N_wedge:
                    for state in states:
                        count_state_action_mask[state,action] = True
        return count_state_action_mask
    else:
        ...
        if self.space == 'Pi_b_SPIBB':
            ...
            for s in range(self.nb_states):
                ...
                if len(q[s, self.mask[s]]) > 0:
                    ...
                    k = np.argmax(q[s, self.mask[s]])
                    m = np.where(self.mask[s])
                    kk = m[0][k]
                    x = pi[s]
                    mf = x[kk]
